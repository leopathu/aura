...
import type { Agent, ConversationSummary, ChatMessageData, AgentSourceChunk } from "@/types";
...
/**
 * Source citations panel — renders clickable links back to the source app.
 */
function AgentSourcesPanel({ sources }: { sources: AgentSourceChunk[] }) {
  if (sources.length === 0) return null;
  return (
    <div className="mt-3 pt-3 border-t border-slate-200 space-y-1">
      <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Sources</p>
      {sources.map((src, i) => (
        <div key={i} className="flex items-start gap-2 text-xs text-slate-500 bg-slate-50 rounded-lg px-2 py-1.5">
          {/* App icon */}
          <div className="w-4 h-4 shrink-0 mt-0.5 relative">
            <Image
              src={`/icons/${src.app_type}.svg`}
              alt={src.app_type}
              width={16}
              height={16}
              className="rounded"
...
/**
 * Streaming chat component for an agent — mirrors BrainChat but uses
 * agent-specific SSE endpoint and renders source URLs with app icons.
 */
export function AgentChat({ agent }: AgentChatProps) {
  ...
  const loadConversations = useCallback(async () => {
    try {
      const list = await agentChatService.listConversations(agent.id);
  ...
  const loadConversation = useCallback(
    async (convId: string) => {
      try {
        const detail = await agentChatService.getConversation(agent.id, convId);
        const msgs: LocalMessage[] = detail.messages.map((m: ChatMessageData) => ({
          id: m.id,
          role: m.role,
          content: m.content,
          ...(m.sources_json
            ? { sources: JSON.parse(m.sources_json) as AgentSourceChunk[] }
            : {}),
        }));
        setMessages(msgs);
  ...
  const handleDelete = async (e: React.MouseEvent, convId: string) => {
    ...
    try {
      await agentChatService.deleteConversation(agent.id, convId);
  ...
  const handleSend = useCallback(() => {
    ...
    const abort = agentChatService.streamChat(
      {
        agent_id: agent.id,
        message: query,
  ...
  }, [input, isStreaming, activeConvId, agent.id, loadConversations]);
        ...
        <div className="flex-1 overflow-y-auto py-1">
          ...
          {conversations.map((conv) => (
            <div
              key={conv.id}
              onClick={() => {
                if (conv.id !== activeConvId) void loadConversation(conv.id);
              }}
              className={cn(
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4 bg-slate-50">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center text-slate-400 py-16">
              ...
              <p className="text-base font-medium text-slate-600">{agent.name}</p>
              <p className="text-sm mt-1 max-w-xs">
                Ask anything about the data synced from this agent&apos;s connected apps.
              </p>
            </div>
          )}
          ...
          {messages.map((msg) => (
            ...
            >
              ...
              >
                ...
                {msg.sources && <AgentSourcesPanel sources={msg.sources} />}
              </div>
            </div>
          ))}
